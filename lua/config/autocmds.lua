...
local list_wins = function()
  ...
  for _, win in ipairs(all) do
    local config = vim.api.nvim_win_get_config(win) -- 窗口配置（判断浮动窗口）
    local buf = vim.api.nvim_win_get_buf(win) -- 窗口对应的 buffer
    local wininfo = vim.fn.getwininfo(win)[1] -- 窗口信息（判断 quickfix）
    -- 判断是否在 ignore 列表里
    ...
    local is_float = config.relative ~= "" -- 是否是浮动窗口
    local is_qf = wininfo.quickfix == 1 or wininfo.loclist == 1 -- 是否是 quickfix/loclist
    if not is_ignore and (is_ft or is_float or is_qf) then
      table.insert(close, win) -- 属于“应该关闭”的窗口
    else
      table.insert(rest, win) -- 其它窗口
    end
  end
