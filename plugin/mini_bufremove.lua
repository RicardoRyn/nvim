...
require("mini.misc").safely("now", function()
  ...
  vim.keymap.set("n", "<leader>ba", function()
    ...
    for _, b in ipairs(vim.api.nvim_list_bufs()) do
      if vim.fn.buflisted(b) == 1 and not ba.is_pinned(b) then
  ...
  vim.keymap.set("n", "<leader>bo", function()
    ...
    for _, b in ipairs(vim.api.nvim_list_bufs()) do
      if vim.fn.buflisted(b) == 1 and b ~= cur and not ba.is_pinned(b) then
        MiniBufremove.delete(b)
      end
    end
  ...
  vim.keymap.set("n", "<leader>bw", function() MiniBufremove.wipeout() end, { desc = "Buffer wipeout" })
